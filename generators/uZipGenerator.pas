...
uses
  System.SysUtils, System.JSON, System.Generics.Collections, System.Math, uBaseGenerator;
...
type
  ...
  private
    ...
    function FindRandomPath(CurrX, CurrY: Integer; Visited: TArray<TArray<Boolean>>; Count: Integer): Boolean;
    procedure ClearGrid;
...
function TOctailyZipGenerator.FindRandomPath(CurrX, CurrY: Integer; Visited: TArray<TArray<Boolean>>; Count: Integer): Boolean;
var
  Dirs: array[0..3] of TPoint;
  I, NextX, NextY, RandIdx: Integer;
  Temp: TPoint;
begin
  if Count = (FGridSize * FGridSize) then Exit(True);

  Dirs[0].X := 0; Dirs[0].Y := -1; Dirs[1].X := 0; Dirs[1].Y := 1;
  Dirs[2].X := -1; Dirs[2].Y := 0; Dirs[3].X := 1; Dirs[3].Y := 0;

  for I := 0 to 3 do begin
    RandIdx := Random(4);
    Temp := Dirs[I]; Dirs[I] := Dirs[RandIdx]; Dirs[RandIdx] := Temp;
  end;

  for I := 0 to 3 do begin
    NextX := CurrX + Dirs[I].X; NextY := CurrY + Dirs[I].Y;
    if (NextX >= 0) and (NextX < FGridSize) and (NextY >= 0) and (NextY < FGridSize)
       and (not Visited[NextX, NextY]) then begin
      Visited[NextX, NextY] := True;
      Temp.X := NextX; Temp.Y := NextY;
      FSolutionPath.Add(Temp);
      if FindRandomPath(NextX, NextY, Visited, Count + 1) then Exit(True);
...
begin
  ...
  for R := 0 to FGridSize - 1 do
    for C := 0 to FGridSize - 1 do begin
      Visited[R, C] := False;
      FGridNumbers[R, C] := 0;
    end;
  ...
  if FindRandomPath(StartPos.X, StartPos.Y, Visited, 1) then begin
    // 1. Durak (Başlangıç)
    ...
    // Son Durak (Bitiş)
    FGridNumbers[FSolutionPath[FSolutionPath.Count-1].X, FSolutionPath[FSolutionPath.Count-1].Y] := WaypointValue;
  end;
  FGameDate := Date;
end;
...
begin
  ...
  for R := 0 to FGridSize - 1 do begin
    RowArr := TJSONArray.Create;
    for C := 0 to FGridSize - 1 do RowArr.Add(FGridNumbers[R, C]);
...
begin
  ...
  try
    ...
    if not Assigned(JSONGuess) then begin
    ...
    if JSONGuess.Count <> (FGridSize * FGridSize) then begin
      Result.AddPair('success', TJSONBool.Create(False));
    ...
    for I := 0 to JSONGuess.Count - 1 do begin
      P.X := JSONGuess.Items[I].GetValue<Integer>('x');
      P.Y := JSONGuess.Items[I].GetValue<Integer>('y');

      // Sınır kontrolü ve Çift ziyaret kontrolü
      if (P.X < 0) or (P.X >= FGridSize) or (P.Y < 0) or (P.Y >= FGridSize) or GridVisited[P.X, P.Y] then begin
        Result.AddPair('success', TJSONBool.Create(False));
      ...
      if I > 0 then begin
        ...
        if not IsAdjacent(P, PrevP) then begin
          Result.AddPair('success', TJSONBool.Create(False));
      ...
      if FGridNumbers[P.X, P.Y] > 0 then begin
        if FGridNumbers[P.X, P.Y] <> ExpectedWaypoint then begin
          Result.AddPair('success', TJSONBool.Create(False));
