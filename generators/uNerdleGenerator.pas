...
function TOctailyNerdleGenerator.GetDebugAnswer: string;
begin
  Result := FDailyEquation; // Hedef kelimeyi doğrudan döndür
end;
...
begin
  ...
  begin
    if AExpr[LPos] = '+' then
      Exit(Evaluate(Copy(AExpr, 1, LPos - 1)) + Evaluate(Copy(AExpr,
        LPos + 1, MaxInt)))
    else
      Exit(Evaluate(Copy(AExpr, 1, LPos - 1)) - Evaluate(Copy(AExpr, LPos + 1,
        MaxInt)));
  end;
  ...
  begin
    ...
    begin
      try
        if Evaluate(Copy(AExpr, LPos + 1, MaxInt)) = 0 then
          Exit(-999999);
        Exit(Evaluate(Copy(AExpr, 1, LPos - 1)) div Evaluate(Copy(AExpr,
          LPos + 1, MaxInt)));
      except
        Exit(-999999);
      end;
...
begin
  ...
  for I := 1 to Length(AExpr) - 1 do
    if (AExpr[I] = '0') and
      ((I = 1) or (CharInSet(AExpr[I - 1], ['+', '-', '*', '/']))) then
...
begin
  ...
  repeat
    ...
    if (res >= 0) and (Length(tempEq) = TARGET_LEN) and
      (not HasLeadingZeros(tempEq)) then
      isValid := True;
